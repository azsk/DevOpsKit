...
# 	SVTStatusReport([string] $tenantId, [InvocationInfo] $invocationContext, [SVTResourceResolver] $resolver): 
...
# 			$sscore = [SubscriptionSecurityStatus]::new($this.TenantContext.tenantId, $this.InvocationContext);
# 			if ($sscore) 
# 			{
...
# 			$secStatus = [ServicesSecurityStatus]::new($this.TenantContext.tenantId, $this.InvocationContext, $this.ServicesResolver);

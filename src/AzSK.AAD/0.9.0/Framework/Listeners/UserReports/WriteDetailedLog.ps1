    ...
    [void] RegisterEvents()
    {
        ...
        $this.RegisterEvent([SVTEvent]::EvaluationStarted, {
            ...
            try 
            {
				...
				{
					$currentInstance.SetFilePath($Event.SourceArgs.TenantContext, $Event.SourceArgs.ResourceContext.ResourceGroupName, ($Event.SourceArgs.FeatureName + ".LOG"));            
				...
				{
					$currentInstance.SetFilePath($Event.SourceArgs.TenantContext, $Event.SourceArgs.TenantContext.TenantName, ($Event.SourceArgs.FeatureName + ".LOG"));            
					$startHeading = ([Constants]::ModuleStartHeadingSub -f $Event.SourceArgs.FeatureName, $Event.SourceArgs.TenantContext.TenantName, $Event.SourceArgs.TenantContext.tenantId);
				}
        ...
        $this.RegisterEvent([SVTEvent]::EvaluationCompleted, {
            ...
            try 
            {
				...
				if($props)
				{
					...
					{
						$currentInstance.AddOutputLog(([Constants]::CompletedAnalysisSub  -f $props.FeatureName, $props.TenantContext.TenantName, $props.TenantContext.tenantId));
					}
				}
				else
				{
            ...
            try 
            {
				...
				else
				{
					$currentInstance.AddOutputLog(("Checking: [{0}]-[$($Event.SourceArgs.ControlItem.Description)] for subscription [{1}]" -f 
                        $Event.SourceArgs.FeatureName, 
                        $Event.SourceArgs.TenantContext.TenantName), 
                    $true);  
				}
            }
            catch 
		...
		$this.RegisterEvent([AzSKRootEvent]::CommandProcessing, {
            ...
            try 
            {
				...
				{
					$currentInstance.SetFilePath($Event.SourceArgs.TenantContext, $Event.SourceArgs.TenantContext.TenantName, "Detailed.LOG");
				...
				if($_.VerificationResult -ne [VerificationResult]::NotScanned)
				{
					...
					{		
						$this.AddOutputLog(("**{3}**: [{0}]-[{2}] for subscription: [{1}]" -f 
								$eventContext.FeatureName, 
								$eventContext.TenantContext.TenantName, 
								$eventContext.ControlItem.Description, 
								$_.VerificationResult.ToString()));     
					}
				}

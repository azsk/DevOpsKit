...
{    
    ...
    hidden GetAADSettings()
    {
        ...
        {
            $this.AADPermissions = [ActiveDirectoryHelper]::InvokeAADAPI("/api/Permissions")
        }

        if ($this.CASettings -eq $null)
        {
            $this.CASettings = [ActiveDirectoryHelper]::InvokeAADAPI("/api/PasswordReset/PasswordResetPolicies")
        }

        if ($this.AdminMFASettings -eq $null)
        {
            $this.AdminMFASettings = [ActiveDirectoryHelper]::InvokeAADAPI("/api/BaselinePolicies/RequireMfaForAdmins")
        }

        if ($this.MFASettings -eq $null)
        {
            $this.MFASettings = [ActiveDirectoryHelper]::InvokeAADAPI("/api/MultiFactorAuthentication/TenantModel")
        }

        if ($this.B2BSettings -eq $null)
        {
            $this.B2BSettings = [ActiveDirectoryHelper]::InvokeAADAPI("/api/Directories/B2BDirectoryProperties")
        }

        if ($this.DeviceSettings -eq $null)
        {
            $this.DeviceSettings = [ActiveDirectoryHelper]::InvokeAADAPI("/api/DeviceSetting")
        }

        if ($this.MFABypassList -eq $null)
        {
            $this.MFABypassList = [ActiveDirectoryHelper]::InvokeAADAPI("/api/MultifactorAuthentication/BypassedUser")
        }


        if ($this.SSPRSettings -eq $null)
        {
            $this.SSPRSettings = [ActiveDirectoryHelper]::InvokeAADAPI("/api/PasswordReset/PasswordResetPolicies")
        }
            
        if ($this.AuthNPasswordPolicySettings -eq $null) 
        {
            $this.AuthNPasswordPolicySettings = [ActiveDirectoryHelper]::InvokeAADAPI("/api/AuthenticationMethods/PasswordPolicy")
        }
    
        if ($this.EnterpriseAppSettings -eq $null)
        {
            $this.EnterpriseAppSettings = [ActiveDirectoryHelper]::InvokeAADAPI("/api/EnterpriseApplications/UserSettings")
        }
    ...
    hidden [ControlResult] CheckNoGuestsInGlobalAdminRole([ControlResult] $controlResult)
	{
        ...
        #TODO: Expand this to other privileged roles (Security Admin, etc. - see ContextHelper)

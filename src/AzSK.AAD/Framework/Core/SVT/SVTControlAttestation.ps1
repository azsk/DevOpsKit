...
class SVTControlAttestation 
{
	...
	hidden [TenantContext] $TenantContext;
    hidden [InvocationInfo] $InvocationContext;

	SVTControlAttestation([SVTEventContext[]] $ctrlResults, [AttestationOptions] $attestationOptions, [TenantContext] $TenantContext, [InvocationInfo] $invocationContext)
	{
		...
		# $this.controlStateExtension = [ControlStateExtension]::new($this.TenantContext, $this.InvocationContext)
	...
	[void] StartControlAttestation()
	{
		...
		try
		{
			...
			foreach($resource in  $filteredControlResults)
			{
				...
				{
					$tenantId = $resourceValue[0].TenantContext.tenantId
					...
					else
					{
						...
						Write-Host $([String]::Format([Constants]::ModuleAttestStartHeadingSub, $resourceValue[0].FeatureName, $resourceValue[0].TenantContext.TenantName, $resourceValue[0].TenantContext.tenantId)) -ForegroundColor Cyan
					}	
					...
					{
						$isSubscriptionScan = $true;
						Write-Host $([String]::Format([Constants]::CompletedAttestAnalysisSub, $resourceValue[0].FeatureName, $resourceValue[0].TenantContext.TenantName, $resourceValue[0].TenantContext.tenantId)) -ForegroundColor Cyan
					}	
				}

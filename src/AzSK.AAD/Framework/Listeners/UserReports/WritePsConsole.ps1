...
class WritePsConsole: FileOutputBase
{
        ...
        $this.RegisterEvent([SVTEvent]::EvaluationCompleted, {
            ...
            try 
            {
				if($Event.SourceArgs -and $Event.SourceArgs.Count -ne 0)
				{
					...
					{
						$currentInstance.WriteMessage(([Constants]::CompletedAnalysisSub  -f $props.FeatureName, $props.TenantContext.TenantName, $props.TenantContext.tenantId), [MessageType]::Update);
					}
				}
            }
            catch 
		...
		$this.RegisterEvent([SVTEvent]::EvaluationError, {
      	...
      	$this.RegisterEvent([SVTEvent]::ControlStarted, {
            ...
            try 
            {
				...
				{
					$AnalysingControlHeadingMsg =([Constants]::AnalysingControlHeadingSub  -f $Event.SourceArgs.FeatureName, $Event.SourceArgs.ControlItem.Description,$Event.SourceArgs.TenantContext.TenantName)
				}
	...
	hidden [void] CommandStartedAction($event)
	{
		...
		$this.SetFilePath($arg.TenantContext, [FileOutputBase]::ETCFolderPath, "PowerShellOutput.LOG");  	

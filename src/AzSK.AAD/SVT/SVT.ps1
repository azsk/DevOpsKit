...
function Get-AzSKAADSecurityStatusTenant
{
	...
	try 
		{
			$resolver = [SVTResourceResolver]::new($TenantId, $true); #pass $true to scan tenant
...
function Get-AzSKAADSecurityStatusUser
{
	...
	try 
		{
			$resolver = [SVTResourceResolver]::new($TenantId, $false); #pass $false to indicate that the scan is for indiv. user
			$resolver.SetScanParameters($ObjectTypes, $MaxObj)

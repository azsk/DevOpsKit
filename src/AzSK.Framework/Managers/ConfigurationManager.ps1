...
class ConfigurationManager {
	...
	hidden static [string] LoadExtensionFile([string] $svtClassName) {
		...
		if (-not ($extensionSVTClassName -as [type])) {
			...
			if ([ConfigurationHelper]::ExtentionNotFoundFlagStore.containsKey($svtClassName) -and [ConfigurationHelper]::ExtentionNotFoundFlagStore[$svtClassName]) {
			...
			if ([string]::IsNullOrWhiteSpace($extensionFilePath)) {
				[ConfigurationHelper]::ExtentionNotFoundFlagStore["$svtClassName"] = $true
			}
		}
		return $extensionFilePath
	}

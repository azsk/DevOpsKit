...
class ControlStateExtension
{
	...
	hidden [bool] ComputeControlStateIndexer()
	{
		...
		while($loopValue -gt 0)
		{
			...
			try
			{
				[AzureRmHelper]::GetStorageBlobContent($AzSKTemp,$this.IndexerBlobName ,$this.IndexerBlobName , $containerName ,$this.StorageAccount.Context)
			...
			if($null -ne $this.ControlStateIndexer -and  $retVal)
			{
				...
				if(($selectedIndex | Measure-Object).Count -gt 0)
				{
					...
					while($loopValue -gt 0)
					{
						...
						try
						{
							[AzureRmHelper]::GetStorageBlobContent($AzSKTemp,$controlStateBlobName ,$controlStateBlobName , $containerName ,$StorageAccount.Context)
	...
	hidden [ControlState[]] GetPersistedControlStates([string] $controlStateBlobName)
	{
		...
		while($loopValue -gt 0)
		{
			...
			try
			{
                ...
                [AzureRmHelper]::GetStorageBlobContent("$AzSKTemp\ExistingControlStates",$controlStateBlobName ,$controlStateBlobName , $containerName ,$StorageAccount.Context)

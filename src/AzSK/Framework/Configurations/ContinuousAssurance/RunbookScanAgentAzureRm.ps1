		...
		if($null -ne $caScanDataBlobObject)
		{
			...
			if(($subsToScan | Where-Object { $_.Status -notin ("COM","ERR")} | Measure-Object).Count -eq 0)
			{
				...
				if($errorredSubsCount -gt 0)
				{
					...
					Write-Output ("SA.o: Scan could not be completed for a total of [$errorredSubsCount] subscription(s).`nSee subscriptions with 'ERR' state in:`n`t $StorageAccountRG -> $($StorageContext.StorageAccountName) -> $CAMultiSubScanConfigContainerName -> Archive -> ActiveScanTracker_<timestamp>.ERR.json.")
				}

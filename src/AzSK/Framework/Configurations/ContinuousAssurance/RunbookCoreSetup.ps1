...
function CheckAndUpdateExpiringPolicyUrl{
	try{
		...
		Export-AzAutomationRunbook -Name $RunbookName -OutputFolder $outputFolderPath `
		-ResourceGroupName $AutomationAccountRG -AutomationAccountName $AutomationAccountName `
		...
		if($isPolicyUrlUpdatedForSASToken)
		{
			...
			$existingRunbook | Remove-AzAutomationRunbook -Force
			...
			Import-AzAutomationRunbook -Name $RunbookName -Description $Description -Type 'PowerShell' `

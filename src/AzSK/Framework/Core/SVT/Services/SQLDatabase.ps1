...
class SQLDatabase: SVTBase
{
	...
	hidden [ControlResult[]] CheckSqlDatabaseTDE([ControlResult] $controlResult)
    {
        ...
        else
        {
			...
			{
				$controlResult.AddMessage([MessageData]::new("TDE enabled for follwoing databases on SQL Server - ["+ $this.ResourceContext.ResourceName +"]",($enabledDB)));
			}
			if(($disabledDB | Measure-Object).Count -gt 0)
			{
				$controlResult.AddMessage([MessageData]::new("TDE disabled for follwoing databases on SQL Server - ["+ $this.ResourceContext.ResourceName +"]",($disabledDB)));
				$controlResult.EnableFixControl = $true
			}

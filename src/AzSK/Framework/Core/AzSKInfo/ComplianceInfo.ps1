        ...
        if (($this.ComplianceScanResult | Measure-Object).Count -gt 0) {
            ...
            if(-not ($UsePreviewBaselineControls -or $UseBaselineControls )){
               ...
               $summary+= "`r`nTotal controls:          " + $totalControlCount
				...
				$this.PublishCustomMessage([Constants]::DoubleDashLine, [MessageType]::Default);
				$this.PublishCustomMessage("Note: The 'excl. grace' column shows count of controls after excluding controls within a 30 day grace window from first scan.", [MessageType]::Default);
                $this.PublishCustomMessage("`r`n`r`n`r`nDisclaimer: `n    a) Compliance summary/control counts may differ slightly from the central telemetry/dashboard due to various timing/sync lags.`n    b)For accurate compliance numbers, you should run a full scan locally at least once every 90 days", [MessageType]::Default);
